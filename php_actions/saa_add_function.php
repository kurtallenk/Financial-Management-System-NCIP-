...
if(isset($_POST['add_btn'])){
	...
	$sql_add = "INSERT INTO specific_budget (program_id, project, uacs_id,responsibility_center, gaasaa, class_category, specific_budget_amount) VALUES ('$program', '$project','$rc','$gaasaa','$classCategory','$budget')";
	...
	if ($connect->query($sql_add) === TRUE) {	
			  ...
			  //UPDATE GAS BUDGET
				//GET GAS BUDGET
				$sql_agency_gas = "SELECT * FROM agency WHERE agency_id=1";
			...
			//PROGRAM BUDGET DEDUCT to AGENCY BUDGET
				...
				$sql_gas_budget_update="UPDATE agency SET agency_budget='$new_agency_budget' WHERE agency_id=1";
				//UPDATE GAS BUDGET
			...
			$date=date('Y-m-d');
			$activity="";
			$activity="Added ".$project." on Sub-Allotments";
			...
			$connect->query($sql_activity_add); 
			  $connect->close();
			  header("Location: ../index.php?o=gas");
			  exit();
	}
	else {
